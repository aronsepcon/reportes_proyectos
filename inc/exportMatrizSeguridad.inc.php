    ...
    foreach ($results as $rs) {
        ...
        foreach ($evidencia as $elemento) {
            if (strlen($elemento) > 0) {
                ...
                $objPHPExcel->setActiveSheetIndex()->setCellValue('K'.$fila,constant("URL") . "photos/" .  $elemento);
                $objPHPExcel->setActiveSheetIndex()->getCell('K' . $fila)->getHyperlink()->setUrl(constant("URL") . "photos/" .  $elemento);
            }
        }
        /*$foto = "../../ssma/public/photos/" . $rs['evidencia'];


        if ($rs['evidencia'] != '' && file_exists($foto)) {
            $objPHPExcel->getActiveSheet()->getRowDimension($fila)->setRowHeight(200);
            $objDrawing = new PHPExcel_Worksheet_Drawing();
            $objDrawing->setCoordinates('K' . $fila);
            $objDrawing->setName($rs['evidencia']);
            $objDrawing->setDescription(constant("URL") . "photos/" . $rs['evidencia']);

            $objDrawing->setPath("../../ssma/public/photos/" . $rs['evidencia']);
